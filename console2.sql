...
(
    ...
    PRO_MNG EMP_NO UNIQUE NOT NULL REFERENCES EMP ON DELETE NO ACTION,
    PRO_DESC CLOB(10)
);
...
(
    ...
    EMP_SAL   SALARY,
    DEPT_NO   DEPT_NO     DEFAULT NULL REFERENCES DEPT ON DELETE SET NULL,
    PRO_NO    PRO_NO      DEFAULT NULL,

    FOREIGN KEY (PRO_NO) REFERENCES PRO (PRO_NO) ON DELETE SET NULL

    /*CONSTRAINT PRO_EMP_NO CHECK ((SELECT COUNT (*) FROM EMP E
        WHERE E.PRO_NO = EMP.PRO_NO) <= 50)*/
);
...
CREATE TABLE DEPT
(
    DEPT_NO DEPT_NO PRIMARY KEY,
    DEPT_EMP_NO INTEGER NO NULL CHECK (DEPT_EMP_NO BETWEEN 1 AND 100),
    DEPT_NAME VARCHAR(200) DEFAULT 'Nameless' NOT NULL,
    DEPT_TOTAL_SAL SALARY DEFAULT 1000000.00 NOT NULL CHECK (DEPT.DEPT_TOTAL_SAL >= 100000.00),
    DEPT_MNG EMP_NO DEFAULT NULL
        REFERENCES EMP ON DELETE SET NULL
        /*CHECK (DEPT_MNG IS NULL OR
               DEPT_MNG IS NOT NULL AND
               ((SELECT COUNT(*) FROM DEPT WHERE DEPT.DEPT_MNG = DEPT_MNG) = 1)),*/
    /*CHECK (DEPT_EMP_NO = (SELECT COUNT(*) FROM EMP WHERE DEPT_NO = EMP.DEPT_NO)),*/
    /*CHECK (DEPT_TOTAL_SAL >= (SELECT SUM(EMP_SAL) FROM EMP WHERE DEPT_NO = EMP.DEPT_NO))*/
);
